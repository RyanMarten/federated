...
def _build_client_update_fn(
    ...
    *,  # Callers should use keyword args for below.
    ...
    tf_dataset_type: computation_types.SequenceType,
    model_weights_type: computation_types.Type,
  ...
  Returns:
    A `tff.Computation` for local model optimization.
  """

  @tf.function
  def client_update(model, metrics, client_loss, dataset, initial_weights,
                    client_optimizer, reconstruction_optimizer):
    """Updates client model.

    Update computation can be divided into two stages: (1) reconstruction of
    local variables with global variables frozen and (2) training of global
    variables with local variables frozen.

    Outputted weight deltas represent the difference between final global
    variables and initial ones. If using weighted aggregation, the client weight
    (used in aggregation across clients) is the sum of the number of examples
    across all batches in step (2) (that is, only the local steps that involve
    updating global variables).

    Args:
      model: A `tff.learning.reconstruction.Model`.
      metrics: A List of `tf.keras.metrics.Metric`s containing metrics to be
        computed and aggregated across clients.
      client_loss: A `tf.keras.losses.Loss` used to compute batch loss on
  ...
  @computations.tf_computation(tf_dataset_type, model_weights_type)
  def client_delta_tf(tf_dataset, initial_model_weights):
    ...
    return client_update(model, metrics, client_loss, tf_dataset,
...
def _build_run_one_round_fn(
    ...
    federated_server_state_type: computation_types.Type,
    federated_dataset_type: computation_types.SequenceType,
...
def build_training_process(
  ...
  Args:
    ...
    dataset_split_fn: A `reconstruction_utils.DatasetSplitFn` taking in a client
      ...
      reconstruction, and the second is iterated over post-reconstruction. This
      can be used to preprocess datasets to e.g. iterate over them for multiple
      epochs or use disjoint data for reconstruction and post-reconstruction. If
      None, `reconstruction_utils.simple_dataset_split_fn` is used, which
      results in iterating over the original client data for both phases of
      training. See `reconstruction_utils.build_dataset_split_fn` for options.
    client_weighting: A value of `tff.learning.ClientWeighting` that specifies a
      built-in weighting method, or a callable that takes the local metrics of
      the model and returns a tensor that provides the weight in the federated
  ...
  federated_dataset_type = computation_types.at_clients(tf_dataset_type)
